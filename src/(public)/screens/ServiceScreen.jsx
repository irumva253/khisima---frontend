...
const ServiceScreen = () => {
  ...
  // Filter and sort services - SHOW ONLY PUBLISHED
  const filteredAndSortedServices = useMemo(() => {
    if (!servicesData?.data) return [];
    
    // Filter services to show only published ones
    let filtered = servicesData.data.filter(service => {
      ...
      const matchesSearch = searchQuery === '' || 
        ...
        service.smallDescription?.toLowerCase().includes(searchQuery.toLowerCase());
      ...
      if (typeof service.category === 'string') {
        ...
        serviceCategoryId = service.category._id;
      }
    ...
    return filtered;
  }, [servicesData?.data, searchQuery, selectedCategory, sortBy]);
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
        <div className="max-w-md w-full">
          <ErrorMessage
            ...
            message={servicesError?.data?.message || "Failed to load services"}
  ...
  return (
    <>
    <Meta title="Our Services" description="Discover our comprehensive range of professional services designed to meet your needs. From consultation to implementation, we've got you covered." />
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
          ...
          <div className="text-center animate-slide-up">
            ...
            <div className="flex flex-wrap justify-center gap-8 mt-12">
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 animate-fade-in-delay">
                <div className="text-3xl font-bold mb-2">{publishedServices.length}</div>
                <div className="text-blue-100 font-medium">Published Services</div>
              </div>
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 animate-fade-in-delay-3">
                ...
                <div className="text-blue-100 font-medium">Total Services</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 mb-8 border border-gray-100 dark:border-gray-700 animate-fade-in-up">
            ...
            <div className="mt-4 flex justify-between items-center text-sm text-gray-500 dark:text-gray-400">
              <span>
                Showing {filteredAndSortedServices.length} of {publishedServices.length} published services
              </span>
              {searchQuery && (
                <span>
          ...
          {filteredAndSortedServices.length === 0 ? (
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-12 text-center border border-gray-100 dark:border-gray-700 animate-fade-in">
              ...
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-3">No published services found</h3>
              <p className="text-gray-500 dark:text-gray-400 mb-6">
                {searchQuery
                  ? `No published services match your search "${searchQuery}"`
                  : "No published services available in the selected category"
                }
              </p>
              ...
              {filteredAndSortedServices.map((service, index) => (
                ...
                >
                  ...
                  <div className={`relative ${viewMode === 'list' ? 'w-64 flex-shrink-0' : 'h-48'} overflow-hidden`}>
                    ...
                    <div className={`${service.fileKey ? 'hidden' : 'flex'} w-full h-full bg-gradient-to-br from-blue-200 to-indigo-200 dark:from-blue-900/50 dark:to-indigo-900/50 items-center justify-center`}>
                      <div className="text-center text-blue-600 dark:text-blue-400">
                        <svg className="w-12 h-12 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M4 l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2  002 2z" />
                        </svg>
                        <p className="font-medium text-sm">Service Image</p>
                      </div>
                    </div>

                    {/* Status Badge - Only show for published services */}
                    <div className="absolute top-3 right-3">
                      <span className={`px-3 py-1 text-xs font-semibold rounded-full border backdrop-blur-sm ${getStatusBadge(service.status)} shadow-lg`}>
                        Published
                      </span>
                    </div>
                  </div>
                  ...
                  <div className={`p-6 ${viewMode === 'list' ? 'flex-1 flex flex-col justify-between' : ''}`}>
                    <div>
                      {/* Category Tag */}
                      {service.category && categories.find(cat => cat._id === service.category) && (
                        <div className="mb-3">
                          <Link
                          ...
                          >
                            {categories.find(cat => cat._id === service.category)?.title}
                          </Link>
                        </div>
                      )}
                      ...
                      {service.smallDescription && (
                        <p className="text-gray-600 dark:text-gray-300 mb-4 leading-relaxed">
                          {viewMode === 'list' 
                            ? truncateText(service.smallDescription, 200)
                            : truncateText(service.smallDescription, 120)
                          }
                        </p>
                      )}
                    ...
                    <div className="mt-4">
                      <Link
                        to={`/services/${service.category?._id || service.category}`}
            ...
            <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-800 rounded-2xl shadow-2xl p-12 mt-16 text-white text-center relative overflow-hidden animate-fade-in">
              ...
              <div className="relative z-10">
                ...
                <p className="text-blue-100 text-lg mb-8 max-w-2xl mx-auto">
                  Our team can help you find the perfect service for your needs or create a custom solution tailored specifically for you.
                </p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  ...
                  <Dialog>
                    ...
                    <DialogContent className="dark:bg-gray-800 dark:text-white">
                      <DialogHeader>
                        <DialogTitle className="dark:text-white">Request Custom Service</DialogTitle>
                        <DialogDescription className="dark:text-gray-300">
                          Please provide details about your custom service request.
                        </DialogDescription>
                      </DialogHeader>
      ...
      <style jsx>{`
        ...
        .animate-fade-in-delay-2 {
          animation: fade-in .8s ease-out 0.6s both;
        }
